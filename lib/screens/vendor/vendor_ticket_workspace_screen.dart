...
class VendorTicketWorkspaceScreen extends StatefulWidget {
  ...
  @override
  State<VendorTicketWorkspaceScreen> createState() =>
      _VendorTicketWorkspaceScreenState();
}

class _VendorTicketWorkspaceScreenState
    extends State<VendorTicketWorkspaceScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (!_isInitialized) {
      ...
      if (args != null && args is Map<String, dynamic>) {
        ...
        _lastUpdatedText =
            args['updated_at'] ?? args['created_at'] ?? 'No Date';
        ...
        String rawStatus =
            (args['status'] ?? 'in_progress').toString().toLowerCase().trim();
...
// Initialize selected staff from database
        _selectedStaffId = int.tryParse(
            (args['handled_by_vendor_staff_id'] ?? '0').toString());
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        if (freshData != null) {
          ...
          print(
              "✅ VENDOR WORKSPACE DATA SYNCHRONIZED: Responded Time = ${_ticketData['real_minutes_used']}");
        }
      }
    } catch (e) {
  ...
  Future<void> _ambilSejarahAktivitiTiket(String idTiket) async {
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        setState(() {
          _ticketLogs = json.decode(respon.body);
        });
      }
    } catch (e) {
      print("Ralat sejarah vendor: $e");
    }
  }
  ...
  Future<void> _ambilSenaraiStaffVendorTulen() async {
    ...
    String dynamicVendorIdLive =
        (_ticketData['assigned_vendor_id'] ?? _ticketData['vendor_id'] ?? '0')
            .toString();

    //  STEP 2: Kita paksa URL mengepilkan parameter ?vendor_id=  ke PHP Laragon!
    final url = Uri.parse(
        'http://$domain/helpdesk_api/get_vendor_staff.php?vendor_id=$dynamicVendorIdLive&b_cache=${DateTime.now().millisecondsSinceEpoch}');
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        print(
            "LIVE FILTERED VENDOR STAFF LOADED: ${decodedData.length} people for Vendor ID: $dynamicVendorIdLive");
      }
    } catch (e) {
      ...
      setState(() {
        _senaraiStaffVendorLiveDB = [
          {
            "staff_id": 101,
            "full_name": "Syaza Live",
            "role": "Vendor Manager"
          },
          {
            "staff_id": 102,
            "full_name": "Farid Live",
  ...
  // =========================================================================
  // =========================================================================
  Future<void> _hantarResponVendorKePHP(String statusPilihan, String notaMesej,
      {int? staffId}) async {
    setState(() => _isSaving = true);

    final String domain = kIsWeb ? 'localhost' : '10.103.19.67';
    final url =
        Uri.parse('http://$domain/helpdesk_api/update_vendor_action.php');

    int finalStaffIdToSave = staffId ?? _selectedStaffId ?? 102;
    String statusFormatEnumDB =
        statusPilihan.toLowerCase().trim().replaceAll(' ', '_');
    ...
    try {
      ...
      if (waktuMulaSlaStr.isEmpty) {
        waktuMulaSlaStr = _slaData?['responded_at'] ??
      ...
      if (waktuMulaSlaStr.isNotEmpty) {
        ...
        DateTime waktuPassVendorTulen =
            DateTime.utc(year, month, day, hour, minute, second)
                .subtract(const Duration(hours: 8));
        Duration jarakMinitMasaSemasa =
            DateTime.now().toUtc().difference(waktuPassVendorTulen);
        ...
        if (totalMinitDinamik >= 60) {
          ...
          kiraMinitDinamikDart =
              "${totalMinitDinamik <= 0 ? 1 : totalMinitDinamik}m";
        }
      }
    } catch (e) {
      ...
      if (respon.statusCode == 200) {
        ...
        if (hasil['status'] == 'berjaya') {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              ...
              content: Text('DB Denied: ${hasil['mesej']}')));
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            backgroundColor: Colors.red,
            content: Text('Server Error: Code ${respon.statusCode}')));
      }
    } catch (e) {
      print("Error fatal hantaran vendor: $e");
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          backgroundColor: Colors.red,
          content: Text('Connection Failure: • $e')));
    } finally {
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const Text('Confirm the status change you are about to apply.',
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (val) {
                    if (val != null)
                      setDialogState(() => temporaryStatusSelection = val);
                  },
                ),
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  ...
                  Navigator.pop(context);
                  _hantarResponVendorKePHP(
                      temporaryStatusSelection, _remarksMessageController.text);
                },
              )
            ],
          );
  ...
  Widget build(BuildContext context) {
    ...
    try {
      ...
      if (waktuMulaSlaStr.isEmpty) {
        waktuMulaSlaStr =
            _slaData?['responded_at'] ?? _slaData?['submitted_at'] ?? '';
      }
      ...
      if (waktuMulaSlaStr.isNotEmpty && waktuMulaSlaStr.length >= 19) {
        ...
        DateTime waktuMulaSlaTulen =
            DateTime.utc(year, month, day, hour, minute, second)
                .subtract(const Duration(hours: 8));
        DateTime waktuMaksimumDeadlineSla =
            waktuMulaSlaTulen.add(const Duration(hours: 5));

        // 📅 FORMATTING DUE DATE UNTUK TAMPILAN BANNER KUNING ANDA MURNI
        List<String> senaraiBulan = [
          'Jan',
          'Feb',
          'Mar',
          'Apr',
        ...
        DateTime waktuDisplayLokal =
            waktuMaksimumDeadlineSla.add(const Duration(hours: 8));
        ...
        int hourDisplay =
            hourRaw > 12 ? (hourRaw - 12) : (hourRaw == 0 ? 12 : hourRaw);
        String minitStr = waktuDisplayLokal.minute.toString().padLeft(2, '0');
        dueTimeLiveTulenDipapar =
            "$hariStr $bulanStr, $hourDisplay:$minitStr $amPm";
        ...
        Duration bakiMasaDuration =
            waktuMaksimumDeadlineSla.difference(sekarangUtc);
        ...
        if (bakiMasaDuration.isNegative) {
          ...
          jamDanMinitSlaLiveDipapar =
              "${bakiJamDinamik}h ${bakiMinitDinamik}m left";
        }
      }
    } catch (e) {
      jamDanMinitSlaLiveDipapar =
          _slaData?['time_left']?.toString() ?? "5h 0m 0s left";
      dueTimeLiveTulenDipapar =
          _slaData?['due_date']?.toString() ?? "07 Sep, 12:59 PM";
    }
        ...
        builder: (BuildContext context) {
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              title: const Text('Vendor Work Order Workspace',
                  style: TextStyle(
                      color: AppColors.navy,
                      fontSize: 13,
...
// ==================== 📝 TAB 1: DETAILS ====================
                        ListView(
                          ...
                          children: [
                            (() {
                              _ticketData['is_saved_secure'] = (_ticketData[
                                          'real_save_date_time'] !=
                                      null ||
                                  (_ticketData[
                                              'handled_by_vendor_staff_id'] !=
                                          null &&
                                      _ticketData['handled_by_vendor_staff_id']
                                              .toString() !=
                                          '0' &&
                                      _ticketData['handled_by_vendor_staff_id']
                                              .toString() !=
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  (() {
                                    ...
                                    bool adakahVendorDahResponDatabaseLive =
                                        _ticketLogs.any((log) =>
                                                (log['reason'] ?? '')
                                                    .toString()
                                                    .toUpperCase()
                                    ...
                                    if (adakahVendorDahResponDatabaseLive ||
                                        statusLabel == 'CLOSED') {
                                      return Container(
                                        width: double.infinity,
                                        padding: const EdgeInsets.all(12),
                                          ...
                                          children: [
                                            ...
                                            Text(
                                                "Responded in ${(() {
                                                  ...
                                                  return masaDitemui;
                                                })()} · ${_ticketData['real_save_date_time'] ?? _lastUpdatedText}",
                                                style: const TextStyle(
                                                    fontSize: 11.5,
                                                    fontWeight: FontWeight.bold,
                                                    color:
                                            ...
                                            const SizedBox(height: 4),
                                            Text(
                                                'Sent to vendor ${_slaData?['responded_at'] ?? '02 Sep 2026, 2:20 PM'} · 5 working hours to respond',
                                                style: const TextStyle(
                                                    fontSize: 10,
                                                    color: Colors.grey)),
                                          ],
                                      ...
                                      child: Container(
                                        ...
                                        child: _senaraiStaffVendorLiveDB.isEmpty
                                            ? const Padding(
                                                ...
                                                child: Center(
                                                    child: Text(
                                                        "Loading fresh vendor staff from database...",
                                                        style: TextStyle(
                                                            fontSize: 11,
                                                            color: Colors.grey,
                                                            fontStyle: FontStyle
                                                        ...
                                                        onChanged: (_ticketData[
                                                            ...
                                                            : (int? val) {
                                                                if (val !=
                                                                  ...
                                                                  setState(() {
                                                                    _selectedStaffId =
                                                                        val; // Hanya tukar bulatan biru visual tanpa hantar data/pop-up!
                                                                  });
                                                                }
                                                              },
                                                        // =========================================================================
...
// =========================================================================
                                  (() {
                                    bool sudahHantarResponPertama = _ticketLogs
                                            .any((log) => (log['reason'] ?? '')
                                                .toString()
                                                .toUpperCase()
                                                .contains('HANDLED BY: 👤')) ||
                                                  ...
                                                  shape: RoundedRectangleBorder(
                                                      borderRadius:
                                                          BorderRadius.circular(
                                                              8))),
                                              onPressed: () {
                                                String teksUlasanVendorSejati =
                                                    _remarksMessageController
                                                        .text
                                                        .trim();

                                                if (teksUlasanVendorSejati
                                                    .isEmpty) {
                                                  ScaffoldMessenger.of(context)
                                                      .showSnackBar(
                                                    const SnackBar(
                                                      backgroundColor:
                                                          Colors.redAccent,
                                                      content: Text(
                                                          'Error: Please enter a message in the Respond to Submitter box! '),
                                                    ),
                                                  );
                                                  return;
                                                }

                                                String statusDihantar =
                                                    sudahHantarResponPertama
                                                        ? 'Closed'
                                                        : 'In Progress';

                                                if (!sudahHantarResponPertama) {
                                                  int targetStaffIdToSave =
                                                      _selectedStaffId ?? 102;
                                                  setState(() {
                                                    _ticketData[
                                                            'handled_by_vendor_staff_id'] =
                                                        targetStaffIdToSave;
                                                        ...
                                                        content: Column(
                                                          ...
                                                          children: [
                                                            ...
                                                            const SizedBox(
                                                                height: 4),
                                                            const Text(
                                                                'Confirm that you have completed the task and want to close this ticket murni.',
                                                                textAlign:
                                                                    TextAlign
                                                                        .center,
                                                                style: TextStyle(
                                      ...
                                      : Column(
                                          children: List.generate(
                                            ...
                                            String namaAktor =
                                                log['transferred_by_name'] ??
                                                    'SYSTEM';
                                            String deskripsi = log['reason'] ??
                                                'Activity logged.';
                                            String tarikhLog =
                                                log['transferred_at'] ?? '';
                                            bool isStatusChange = deskripsi
                                                .toLowerCase()
                                                .contains('status');
                                            if (tarikhLog.length > 16) {
                                              tarikhLog = tarikhLog.substring(
                                                      11, 16) +
                                                  " " +
                              ...
                              _isFeedbackLoading
                                  ...
                                  : (statusLabel
                                          ...
                                          _ticketData['status']
                                                  ...
                                                  .toUpperCase()
                                                  .trim() !=
                                              'CLOSED') // HURUF BESAR: Kalis ralat string huruf kecil database!
                                      ? _buildLockedFeedbackState() // Jika open / in_progress, paksa tunjuk kad locked!
                                      : (_feedbackData == null ||
                                              _feedbackData?['status'] ==
                                                  'kosong' ||
                                              _feedbackData?['comment'] == null)
