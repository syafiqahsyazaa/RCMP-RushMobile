...
// =========================================================================
  Future<void> _tarikTiketVendorLive({bool isBackground = false}) async {
    ...
    final url = Uri.parse(
        'http://$domain/helpdesk_api/get_vendor_tickets.php?vendor_id=$idVendorSemasa&b_cache=${DateTime.now().millisecondsSinceEpoch}');
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        for (var tiket in dataDisedut) {
          String stat =
              (tiket['status'] ?? 'open').toString().toLowerCase().trim();
          if (stat == 'closed') {
            kaunterClosed++;
          } else {
            kaunterProgress++;
...
// NOTIFIKASI PHONE DIBUANG!
  ...
  Widget _buildNotifItem(dynamic tkt) {
    ...
    return InkWell(
      onTap: () {
        ...
        Navigator.pushNamed(context, '/vendor_ticket_workspace',
            arguments: tkt);
      },
      child: Container(
                ...
                children: [
                  ...
                  if (!isBreached)
                    Padding(
                      ...
                      child: Text(
                          '${tkt['my_department'] ?? 'Dept'} · ${tkt['created_at'] ?? ''}',
                          style: const TextStyle(
  ...
  void _jalankanPenapisSearchMurni() {
    ...
    setState(() {
      _senaraiTiketTapisSearch = _senaraiTiketVendorLive.where((tiket) {
        ...
        if (_statusKadDipilihLive == 'IN_PROGRESS') {
          matchCard = (statRaw !=
              'closed'); // Mengambil data open dan in_progress seacuan pembahagi kaunter
    ...
    print(
        "FILTER STATE SYNCED: Card [$_statusKadDipilihLive] | Search keyword: '$teksCarian'");
  }

  Widget _buildPureUIStatCard(
  ...
  Widget _buildPureUITicketCard(Map<String, dynamic> tiket) {
    ...
    String priorityRaw =
        (tiket['priority'] ?? 'MEDIUM').toString().toUpperCase();
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          CircleAvatar(
              radius: 18,
              backgroundColor: const Color(0xFF0D3B66).withOpacity(0.08),
                ...
                Row(
                  ...
                  children: [
                    Row(children: [
                      ...
                      Text('PRIORITY: $priorityRaw',
                          style: TextStyle(
                              fontSize: 9.5,
                              fontWeight: FontWeight.bold,
                              color: priorityRaw == 'HIGH'
                                  ? Colors.red
                                  : Colors.grey))
                    ]),
                    TextButton.icon(
                        icon: const Icon(Icons.launch_rounded,
                            size: 12, color: Color(0xFF0D3B66)),
...
// Tolong daftarkan route nama '/vendor_ticket_workspace' di fail main.dart
                          final hasilRefresh = await Navigator.pushNamed(
                              context, '/vendor_ticket_workspace',
                              arguments: tiket);
  ...
  Widget _buildWorkOrdersLayoutView(String syarikat) {
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          Text('Work Orders — $syarikat',
              style: TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
                  color: AppColors.textPrimary)),
          const SizedBox(height: 4),
          const Text('Tickets assigned to your company by staff.',
                ...
                child: InkWell(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(
                          color: _statusKadDipilihLive == 'ALL'
              ...
              Expanded(
                child: InkWell(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      border: Border.all(
                          color: _statusKadDipilihLive == 'IN_PROGRESS'
                              ? const Color(0xFFC9A227)
                              : Colors.transparent,
                          width: _statusKadDipilihLive == 'IN_PROGRESS'
                              ? 2.0
                              : 0.0),
                      borderRadius: BorderRadius.circular(12),
                    ),
                    child: _buildPureUIStatCard(
                        value: '$_progressTickets',
                        label: 'PROGRESS',
                        subLabel: 'ACTIVE',
              ...
              Expanded(
                child: InkWell(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      border: Border.all(
                          color: _statusKadDipilihLive == 'CLOSED'
                              ? const Color(0xFF2E9E52)
                              : Colors.transparent,
            ...
            children: [
              Expanded(
                  child: Container(
                      ...
                      child: TextField(
                          controller:
                              _searchController, // memanggil controller asal anda!
                          decoration: const InputDecoration(
                              hintText:
                                  'Search by ticket ID, department, title...',
                              hintStyle:
              ...
              : _senaraiTiketTapisSearch.isEmpty
                  ? const Center(
                      child: Padding(
                          ...
                          child: Text("No tickets assigned to your workspace.",
                              style:
    ...
    return Scaffold(
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Text(
                                        '${_senaraiTiketVendorLive.where((t) => (t['status'] ?? '').toString().toLowerCase() != 'closed').length} active',
                                        style: const TextStyle(
                                            fontSize: 9,
                                            fontWeight: FontWeight.bold,
                                            color: AppColors.navy)),
                              ...
                              _senaraiTiketVendorLive
                                  ...
                                  : SizedBox(
                                      ...
                                      child: ListView.builder(
                                        itemCount: _senaraiTiketVendorLive
                                            .where((t) =>
                                                (t['status'] ?? '')
                                                    .toString()
                                                    .toLowerCase() !=
                                        ...
                                        itemBuilder: (context, idx) {
                                          final tkt = _senaraiTiketVendorLive
                                              .where((t) =>
                                                  (t['status'] ?? '')
                                                      .toString()
                                                      .toLowerCase() !=
      ...
      body: _isLoading
          ...
          : _vendorData['first_login'] == 1 || _vendorData['first_login'] == '1'
              ? const Center(
                  child: Text("Awaiting profile configuration validation...",
