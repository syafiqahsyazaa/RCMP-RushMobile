...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (args != null && _namaAdminLive.isEmpty) {
      setState(() {
        _namaAdminLive = (args['full_name'] ?? args['name'] ?? "").toString().trim();
      });
    }
  }
  ...
  Future<void> _ambilDataAdminDashboard({bool isBackground = false}) async {
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        final Map<String, dynamic>? argsBackup = ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;
        String namaAsalLoginArguments = (argsBackup?['full_name'] ?? argsBackup?['name'] ?? "").toString().trim();
                ...
                if (_incomingTickets.isNotEmpty)
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Text(
                        _incomingTickets.length.toString(),
                ...
                PopupMenuItem<void>(
                  ...
                  child: StatefulBuilder(
                    builder: (BuildContext context, StateSetter setMenuState) {
                      ...
                      int currentTab = int.parse(_counters['notif_tab_active_index']!);
                      ...
                      if (currentTab == 0) {
                        ...
                        filteredNotifs = _incomingTickets.where((t) => (t['remarks'] ?? '').toString().toLowerCase().trim() == _namaAdminLive.toLowerCase().trim()).toList();
                      } else if (currentTab == 2) {
                        filteredNotifs = _incomingTickets.where((t) {
                          try {
                            DateTime createdTime = DateTime.parse(t['created_at'] ?? '');
                        ...
                        child: Column(
                          ...
                          children: [
                            Padding(
                              ...
                              child: Row(
                                children: [
                                  const Text('Notifications', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: AppColors.navy)),
                                  const Spacer(),
                                  Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                                    decoration: BoxDecoration(color: const Color(0xFFEAF1FB), borderRadius: BorderRadius.circular(10)),
                                    child: Text('${_incomingTickets.length} active', style: const TextStyle(fontSize: 10, color: Color(0xFF2F5FA3), fontWeight: FontWeight.bold)),
                                  )
                                ],
                              ),
                            ),
                            Row(
                              children: [
                                _buildMiniTab(0, 'All', currentTab, () => setMenuState(() => _counters['notif_tab_active_index'] = '0')),
                                _buildMiniTab(1, 'Assigned to Me', currentTab, () => setMenuState(() => _counters['notif_tab_active_index'] = '1')),
                                _buildMiniTab(2, 'Deadlines', currentTab, () => setMenuState(() => _counters['notif_tab_active_index'] = '2')),
                              ],
                            ),
                            ...
                            if (filteredNotifs.isEmpty)
                              const Padding(
                                ...
                                child: Text("No relevant notifications!", style: TextStyle(fontSize: 11, color: Colors.grey, fontStyle: FontStyle.italic)),
                              )
                            else
                              SizedBox(
                                height: 350,
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, idx) {
                                    ...
                                    try {
                                      int mins = _kiraMinitBekerjaSLA(DateTime.parse(tkt['created_at'] ?? ''), DateTime.now());
                                      isBreached = mins > 480;
                                    } catch (e) {}
        ...
        child: _isLoading
          ...
          children: [
            ...
            // === BAHAGIAN QUICK ACCESS PANEL DI DALAM ADMIN DASHBOARD ANDA ===
             const Text('Quick Access Panel', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
            const SizedBox(height: 12),

            _QuickLink(icon: Icons.people_outline, label: 'All Tickets',
                onTap: () => Navigator.pushNamed(context, '/admin/tickets',
                    arguments: {
                      ...
                      "full_name": _namaAdminLive.isNotEmpty ? _namaAdminLive : "Admin UniKL",
                      "role": "Admin"
                    })),

            _QuickLink(icon: Icons.people_outline, label: 'Manage Users',
            ...
            _QuickLink(icon: Icons.category_outlined, label: 'Categories',
            ...
            _QuickLink(icon: Icons.bar_chart_outlined, label: 'Reports & Analytics',
              ...
              child: Row(
                ...
                children: data.map((item) {
                  ...
                  if (label == 'Open') {
                    ...
                    dynamicGradientColors = [const Color(0xFFFBBF24), const Color(0xFFD97706)]; // Kuning/Oren Amaran
...
// Carta Bar Melintang Bertingkat Berprofil Kad (Horizontal SaaS-Style Card Bars)
  ...
  // =========================================================================
  Widget _buildMiniTab(int index, String label, int active, VoidCallback onTap) {
    ...
    return Expanded(
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Text(label, textAlign: TextAlign.center, style: TextStyle(fontSize: 11, fontWeight: isSelected ? FontWeight.bold : FontWeight.w500, color: isSelected ? const Color(0xFF2F5FA3) : Colors.grey)),
        ),
      ),
    );
  }
                ...
                children: [
                  ...
                  if (!isBreached)
                    Padding(
                      ...
                      child: Text('${tkt['department'] ?? 'Dept'} · ${tkt['created_at'] ?? ''}', style: const TextStyle(fontSize: 9, color: Colors.black26)),
                    ),
                ],
              ),
            ),
