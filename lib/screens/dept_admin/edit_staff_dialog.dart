...
class _EditStaffDialogContent extends StatefulWidget {
  ...
  @override
  State<_EditStaffDialogContent> createState() =>
      _EditStaffDialogContentState();
}
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          if (!_categoriesOptions.contains(_selectedCategory)) {
            _selectedCategory =
                _categoriesOptions.isNotEmpty ? _categoriesOptions.first : null;
          }
        });
      }
    } catch (e) {
  ...
  Future<void> _saveStaffEdits() async {
    if (_codeController.text.trim().isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Staff Code and Name fields cannot be blank!')));
      return;
    }
        ...
        if (resData['status'] == 'berjaya') {
          if (mounted) {
            ...
            ScaffoldMessenger.of(context)
                .showSnackBar(SnackBar(content: Text(resData['mesej'])));
          }
        }
      }
    } catch (e) {
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  const Text('Edit User Profile & Settings',
                      style: TextStyle(
                          fontSize: 14,
              ...
              const SizedBox(height: 12),
              LabeledDropdown(
                label: 'Assigned Category Operations',
                hint: _selectedCategory ?? 'Select operational track',
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  icon: Icon(_isActive ? Icons.check_circle : Icons.cancel,
                      size: 16,
                      color: _isActive
                          ? const Color(0xFF2E9E52)
                          : const Color(0xFFD64545)),
                  label: Text(
                    _isActive
                        ? 'ACCOUNT STATUS: ACTIVE (Click to Change)'
                        : 'ACCOUNT STATUS: INACTIVE (Click to Change)',
                    style: TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.bold,
                        color: _isActive
