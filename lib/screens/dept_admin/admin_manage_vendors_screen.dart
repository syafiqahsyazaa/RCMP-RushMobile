...
class AdminManageVendorsScreen extends StatefulWidget {
  ...
  @override
  State<AdminManageVendorsScreen> createState() =>
      _AdminManageVendorsScreenState();
}
...
class _AdminManageVendorsScreenState extends State<AdminManageVendorsScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (!_isDeptInitializedLive) {
      try {
        final Map<String, dynamic>? argumentsPetaLogin =
            ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;
        setState(() {
          _currentEmailLive =
              (argumentsPetaLogin?['email'] ?? currentLoggedInUserEmail)
                  .toString()
                  .trim();
          String labelJabatanMentah =
              (argumentsPetaLogin?['departmentLabel'] ?? 'Loading...')
                  .toString();
  ...
  Future<void> _tarikDataVendorsDariLaragon() async {
    ...
    if (resolvedDept == 'Loading...') {
      try {
        final deptUrl = Uri.parse(
            'http://$domain/helpdesk_api/get_admin_dashboard.php?email=$_currentEmailLive');
        ...
        if (deptRespon.statusCode == 200) {
          ...
          String fullLabel =
              deptData['department_label'] ?? 'Unknown Department';
          ...
          setState(() {
            _namaJabatanLive = resolvedDept;
          });
        }
      } catch (e) {
        print("Gagal tarik info jabatan fallback: $e");
      }
    }

    final url = Uri.parse(
        'http://$domain/helpdesk_api/get_vendors.php?email=$_currentEmailLive&department=${Uri.encodeComponent(resolvedDept)}&b_cache=${DateTime.now().millisecondsSinceEpoch}');
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        setState(() {
          ...
          _countPending =
              dataDiterima.where((v) => v['status'] == 'pending').length;
          _countActive = dataDiterima
              .where(
                  (v) => v['status'] == 'active' || v['status'] == 'approved')
              .length;
          _countSuspended =
              dataDiterima.where((v) => v['status'] == 'suspended').length;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  void _jalankanPenapisanDinamikLive() {
    ...
    setState(() {
      _senaraiVendorsDipaparkan = _senaraiSemuaVendors.where((vendor) {
        ...
        bool matchSearch = (vendor['company_name'] ?? '')
                .toString()
                .toLowerCase()
                .contains(kataKunci) ||
            (vendor['email'] ?? '')
        ...
        String statusSyarikatRaw =
            (vendor['status'] ?? 'active').toString().toLowerCase().trim();
        bool matchStatusCard = true;

        if (_selectedStatusFilter == 'Pending') {
          matchStatusCard = (statusSyarikatRaw == 'pending');
        } else if (_selectedStatusFilter == 'Active') {
          matchStatusCard = (statusSyarikatRaw == 'active' ||
              statusSyarikatRaw == 'approved');
        } else if (_selectedStatusFilter == 'Suspended') {
          matchStatusCard = (statusSyarikatRaw == 'suspended' ||
              statusSyarikatRaw == 'inactive');
        }
    ...
    print(
        "VENDOR MATRIX SYNCED: Card [$_selectedStatusFilter] | Keyword: '$kataKunci'");
  }
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(
              'Are you sure you want to remove this vendor from $_namaJabatanLive?'),
          actions: [
            TextButton(
  ...
  Future _eksekusiUpdateVendorKeLaragon(
      {required String id,
      ...
      required String pPos,
      required String pPhn}) async {
    final String domain = kIsWeb ? 'localhost' : '10.0.2.2';
    final url =
  ...
  Future _eksekusiPadamVendorDariLaragon(String id, String nama) async {
    final String domain = kIsWeb ? 'localhost' : '10.0.2.2';
    final url = Uri.parse(
        'http://$domain/helpdesk_api/delete_vendor.php?vendor_id=$id');
    try {
      final respon = await http.get(url);
      if (respon.statusCode == 200) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('$nama removed.')));
        _tarikDataVendorsDariLaragon();
      }
    } catch (e) {
      print("Error: $e");
  ...
  void _pamerDialogEditVendor(Map<String, dynamic> vendorSemasa) {
    final companyNameCtrl =
        TextEditingController(text: vendorSemasa['company_name']);
    final addressCtrl =
        TextEditingController(text: vendorSemasa['address'] ?? '');
    final cityCtrl = TextEditingController(text: vendorSemasa['city'] ?? '');
    final stateCtrl = TextEditingController(text: vendorSemasa['state'] ?? '');
    final postcodeCtrl =
        TextEditingController(text: vendorSemasa['postcode'] ?? '');
    final phoneCtrl = TextEditingController(text: vendorSemasa['phone'] ?? '');
    final picNameCtrl =
        TextEditingController(text: vendorSemasa['pic_name'] ?? '');
    final picPositionCtrl =
        TextEditingController(text: vendorSemasa['pic_position'] ?? '');
    final picPhoneCtrl =
        TextEditingController(text: vendorSemasa['pic_phone'] ?? '');
    final newPasswordCtrl = TextEditingController();
    String statusSemasaLive =
        (vendorSemasa['status'] ?? 'active').toString().toLowerCase() ==
                'suspended'
            ? 'Suspended'
            : 'Active';
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          const Expanded(
                              child: Text('Edit Vendor Profile',
                                  style: TextStyle(
                                      fontSize: 14,
                          ...
                          ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              _eksekusiUpdateVendorKeLaragon(
                                ...
                                statusText: statusSemasaLive == 'Active'
                                    ? 'active'
                                    : 'suspended',
              ...
              children: [
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          border: Border.all(
                              color: _selectedStatusFilter == 'All Status'
                                  ? AppColors.navy
                                  : Colors.transparent,
                              width: 2),
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          border: Border.all(
                              color: _selectedStatusFilter == 'Suspended'
                                  ? Colors.red
                                  : Colors.transparent,
                              width: 2),
                          borderRadius: BorderRadius.circular(12),
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    onChanged: (_) =>
                        _jalankanPenapisanDinamikLive(), // Auto-tapis sewaktu menaip huruf
                ...
                _senaraiVendorsDipaparkan.isEmpty
                    ? Container(
                        ...
                        child: const Column(
                          children: [
                            ...
                            SizedBox(height: 8),
                            Text('No matching vendors found.',
                                style: TextStyle(
                                    fontSize: 12,
                    ...
                    : Column(
                        children: _senaraiVendorsDipaparkan.map((vendor) {
                          String compName =
                              vendor['company_name'] ?? 'Syarikat Vendor';
                          String compMail = vendor['email'] ?? 'No Email';
                          String compPhone = vendor['phone'] ?? 'No Phone';
                          String statusMentahDariDatabase = (vendor['status'] ??
                          ...
                          if (statusMentahDariDatabase == 'suspended' ||
                              statusMentahDariDatabase == 'inactive' ||
                              statusMentahDariDatabase == 'suspend') {
                          ...
                          if (tarikhMentah.length > 10) {
                            tarikhDipapar = tarikhMentah.substring(8, 10) +
                                "/" +
                                tarikhMentah.substring(5, 7) +
                                "/" +
                                tarikhMentah.substring(0, 4);
                          }
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          Container(
                                            ...
                                            decoration: BoxDecoration(
                                                color: statusSyarikat ==
                                                        'ACTIVE'
                                                    ? const Color(
                                                        0xFFEAF7EE) // Hijau Lembut untuk Active
                                                    : const Color(
                                                        0xFFFDF2F2), // Merah Lembut untuk Suspended/Inactive!
                                                borderRadius:
                                                    BorderRadius.circular(6)),
                                            child: Text(statusSyarikat,
                                                style: TextStyle(
                                                    fontSize: 9,
                                                    fontWeight: FontWeight.bold,
                                                    color: statusSyarikat ==
                                                            'ACTIVE'
                                                        ? const Color(
                                                            0xFF2E9E52) // Teks Hijau
                                                        : const Color(
                                                            0xFFD64545) // Teks Merah
                                                    )),
                                          ),
                                        ],
                                      ),
                                            ...
                                            children: [
                                              ...
                                              IconButton(
                                                ...
                                                constraints:
                                                    const BoxConstraints(),
                                                onPressed: () {
                                                  _pamerSiasatSahkanDeleteDialog(
                                                      vendor['vendor_id']
                                                          .toString(),
                                                      compName);
                                                },
