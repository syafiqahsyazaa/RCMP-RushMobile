import 'package:flutter/material.dart';import 'package:flutter/foundation.dart';import 'package:http/http.dart' as http;import 'dart:convert';import '../../theme/app_theme.dart';import '../../theme/admin_theme.dart';
...
final List<String> _ratingLabels = ['Very Dissatisfied', 'Dissatisfied', 'Average Support', 'Satisfied', 'Very Satisfied'];
...
String ticketIdTulen = (argsLive['ticket_id'] ?? '').toString().trim();
...
print("🚀 SEJARAH TIMELINE USER BERJAYA DISEDUT: ${dataDiterima.length} logs untuk tiket $idTiket");
}
} catch (e) {
print("Ralat menarik sejarah aktiviti tiket mata user: $e");
  ...
  Future<void> _ambilDataAduanTerkini(String idTiket) async {
    final String domain = kIsWeb ? 'localhost' : '10.103.19.67';
    final url = Uri.parse('http://$domain/helpdesk_api/get_my_complaints.php?ticket_id=$idTiket&b_cache=${DateTime.now().millisecondsSinceEpoch}');
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        print(" REKOD COMPLAINTS REFRESHED DARI LIST PHP: Handled By = ${_ticketData['remarks']}");
      }
    } catch (e) {
  ...
  Future<void> _cekFeedbackSediaAda(String idTiket) async {
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        if (data != null && data['status'] != 'kosong' && data['status'] != 'locked') {
          if (mounted) {
            setState(() {
    ...
    } catch (e) { print("Error semak feedback: $e"); }
  }
  ...
  Widget _buildSubmittedFeedbackCard() {
    ...
    if (dateStr.length >= 19) {
      try {
        ...
        List<String> listBln = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      ...
                      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF2E9E52)),
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        Text("$rating/5", style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.black38)),
                      ],
                    ),
                  ],
                ),
...
case 'closed': return const Color(0xFF2E9E52);
case 'in_progress': return Colors.orange;
default: return const Color(0xFF2F5FA3);
}
}

Color _getStatusBg(String status) {
switch (status.toLowerCase()) {
case 'closed': return const Color(0xFFEAF7EE);
case 'in_progress': return const Color(0xFFFFF3E0);
default: return const Color(0xFFEAF1FB);
}
}
...
if (mounted) {
ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(hasil['mesej'] ?? 'Thank you for your feedback!')));

if (hasil['status'] == 'berjaya') {
_cekFeedbackSediaAda(ticketId); // tarik semula data feedback dari server
}
}
}
} catch (e) {
ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Ralat penghantaran: $e")));
} finally {
...
Map<String, dynamic> ticketData = (args != null && args is Map<String, dynamic>) ? args : {};
...
String description   = ticketData['description'] ?? 'No description provided.';
...
if (staffHandledBy.isEmpty || staffHandledBy.trim().toLowerCase() == 'null') {
...
title: const Text('Ticket Detail', style: TextStyle(color: AppColors.navy, fontSize: 14, fontWeight: FontWeight.w700)),
),
body: SafeArea(
child: ListView(
padding: const EdgeInsets.all(16),
...
const SizedBox(width: 8),
StatusTag(label: status.toUpperCase(), background: _getStatusBg(status), foreground: _getStatusColor(status)),
],
),
const SizedBox(height: 4),
Text(ticketId, style: const TextStyle(fontSize: 11, color: AppColors.textMuted)),
const SizedBox(height: 16),
Wrap(
spacing: 20,
runSpacing: 14,
children: [
ReviewField(label: 'Department In Charge', value: namaJabatanInCharge),
ReviewField(label: 'From Department', value: myDepartment),
ReviewField(label: 'Priority', value: priority),
ReviewField(label: 'Submitted Date', value: dateSubmitted),
  ReviewField(
    label: 'Last Updated',
    value: (_ticketData['updated_at'] ?? ticketData['updated_at'] ?? dateSubmitted ?? 'Just Now').toString().trim(),
  ),
],
),
const SizedBox(height: 16),
...
Row(
children: const [
Icon(Icons.engineering_outlined, size: 16, color: AppColors.textMuted),
...
crossAxisAlignment: CrossAxisAlignment.start,
children: [
Text(staffHandledBy, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
],
),
),
],
...
Row(children: const [Icon(Icons.chat_bubble_outline_rounded, size: 15, color: Colors.blueGrey),SizedBox(width: 8),Text('YOUR FEEDBACK', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.blueGrey, letterSpacing: 0.5)),],),const SizedBox(height: 12),_hasSubmittedBefore? _buildSubmittedFeedbackCard()
    : Column(crossAxisAlignment: CrossAxisAlignment.start,children: [
const Text('Rate your experience with this ticket resolution process', style: TextStyle(fontSize: 11, color: AppColors.textSecondary)),const SizedBox(height: 12),Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,children: List.generate(_emojis.length, (index) {bool isSelected = _selectedRatingIndex == index;return _FaceButton(emoji: _emojis[index],isSelected: isSelected,onTap: () {setState(() { _selectedRatingIndex = index; });},);}),),
const SizedBox(height: 14),if (_selectedRatingIndex != -1)Center(child: Text(_ratingLabels[_selectedRatingIndex], style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.navy))),const SizedBox(height: 14),const Text('Share your thoughts (Optional)', style: AppTextStyles.fieldLabel),const SizedBox(height: 6),TextField(controller: _feedbackController,minLines: 2, maxLines: 4,style: const TextStyle(fontSize: 12),decoration: InputDecoration(hintText: 'Share your thoughts about response time, staff helpfulness, or equipment restoration accuracy...',hintStyle: const TextStyle(fontSize: 11, color: AppColors.textMuted),filled: true, fillColor: AppColors.fieldBackground,contentPadding: const EdgeInsets.all(10),border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: AppColors.border)),enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: AppColors.border)),focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: AppColors.navy)),),
),const SizedBox(height: 14),_isSaving? const Center(child: CircularProgressIndicator()): SuccessButton(label: 'Submit Official Feedback',icon: Icons.send_outlined,onPressed: () => _submitFeedback(ticketId),),],)
] else ...[
Container(width: double.infinity,padding: const EdgeInsets.all(12),decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.blue.shade100)),child: Row(children: [Icon(Icons.info_outline, size: 16, color: Colors.blue.shade700),const SizedBox(width: 10),Expanded(child: Text('Feedback tracking and rating buttons will automatically activate once this ticket is fully resolved and CLOSED by the staff member.',style: TextStyle(fontSize: 11.5, color: Colors.blue.shade900, fontWeight: FontWeight.w500, height: 1.3),),),],),),
],
],
),
),
...
// =========================================================================
Widget _buildMessagesTimelinePanelLive(Map<String, dynamic> ticketData, String staffName) {
String statusLive = (ticketData['status'] ?? 'open').toString().toLowerCase();
return Container(
width: double.infinity,
padding: const EdgeInsets.all(16),
decoration: BoxDecoration(
...
Row(
children: const [
Icon(Icons.chat_bubble_outline_rounded, size: 16, color: AppColors.textPrimary),
SizedBox(width: 8),
Text(
'MESSAGES & UPDATES',
style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.textPrimary, letterSpacing: 0.5),
),
],
),
Container(
padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3),
decoration: BoxDecoration(color: const Color(0xFF0F52BA), borderRadius: BorderRadius.circular(10)),
child: Text(
'${_ticketLogs.length}',
...
String namaAktor = (log['transferred_by_name'] ?? 'SYSTEM').toString().trim();
String ulasanLogText = (log['reason'] ?? 'Activity logged.').toString().trim();
String tarikhLog = (log['transferred_at'] ?? '').toString();
String masaDipapar = tarikhLog.length > 16 ? tarikhLog.substring(11, 16) : 'Just Now';
String tarikhDipapar = tarikhLog.length > 10 ? tarikhLog.substring(8, 10) + "/" + tarikhLog.substring(5, 7) : '';
if (ulasanLogText.contains('|')) {
ulasanLogText = ulasanLogText.split('|').last.trim();
}
bool adakahIniPelajarComplainant = namaAktor.toLowerCase().contains('student') ||
namaAktor.toLowerCase().contains('you') ||
namaAktor.toLowerCase() == (ticketData['submitter_email'] ?? '').toString().toLowerCase().trim();
if (adakahIniPelajarComplainant) {
return _buildUserChatBubbleRight(ulasanLogText, "$masaDipapar ($tarikhDipapar)");
} else {
return _buildAdminChatBubbleLeft(namaAktor, ulasanLogText, "$masaDipapar ($tarikhDipapar)");
}
}),
),
const SizedBox(height: 16),
...
decoration: BoxDecoration(
color: isClosed ? const Color(0xFFEAF7EE) : const Color(0xFFEAF1FB),
borderRadius: BorderRadius.circular(20),
border: Border.all(color: isClosed ? const Color(0xFF2E9E52).withOpacity(0.2) : const Color(0xFF2F5FA3).withOpacity(0.1)),
),
child: Row(
mainAxisSize: MainAxisSize.min,
children: [
Container(width: 6, height: 6, decoration: BoxDecoration(shape: BoxShape.circle, color: isClosed ? const Color(0xFF2E9E52) : const Color(0xFF0F52BA))),
const SizedBox(width: 8),
Text(text, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
],
),
),
const SizedBox(width: 8),
...
crossAxisAlignment: CrossAxisAlignment.end,
children: [
const Text('You', style: TextStyle(fontSize: 10.5, color: Colors.grey, fontWeight: FontWeight.bold)),
const SizedBox(height: 4),
Container(
...
child: Text(message, style: const TextStyle(color: Colors.white, fontSize: 12, height: 1.3)),
),
],
),
const SizedBox(width: 6),
CircleAvatar(radius: 12, backgroundColor: Colors.blue.shade100, child: const Text('Y', style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: Color(0xFF0F52BA)))),
],
),
);
}
...
crossAxisAlignment: CrossAxisAlignment.start,
children: [
CircleAvatar(radius: 14, backgroundColor: const Color(0xFFF3E8FF), child: const Icon(Icons.engineering_outlined, size: 10,)),
const SizedBox(width: 8),
Expanded(
child: Column(
crossAxisAlignment: CrossAxisAlignment.start,
...
Row(
children: [
Text(actor, style: const TextStyle(fontSize: 11, color: Colors.purple, fontWeight: FontWeight.bold)),
const SizedBox(width: 6),
Text('• $time', style: const TextStyle(fontSize: 10, color: Colors.grey)),
],
),
...
child: Text(message, style: const TextStyle(color: AppColors.textPrimary, fontSize: 12, height: 1.3)),
),
],
),
),
...
const _FaceButton({required this.emoji, required this.isSelected, required this.onTap});
@override
Widget build(BuildContext context) {
return InkWell(
onTap: onTap,
...
border: Border.all(color: isSelected ? AppColors.navy : AppColors.border, width: 1.5),
