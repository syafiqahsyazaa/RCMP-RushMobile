...
class ComplaintConfirmSubmissionsScreen extends StatefulWidget {
  ...
  @override
  State<ComplaintConfirmSubmissionsScreen> createState() => _ComplaintConfirmSubmissionsScreenState();
}

class _ComplaintConfirmSubmissionsScreenState extends State<ComplaintConfirmSubmissionsScreen> {
...
? _safeLiveEmail.split('@')[0].toUpperCase().replaceAll('.', ' ').replaceAll('_', ' ')
    : "USER COMPLAINANT";
...
return AlertDialog(
title: const Text('Remove Item', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.navy)),
...
const SnackBar(content: Text('Complaint item removed successfully.')),
);
},
...
builder: (BuildContext context) {
return AlertDialog(
title: const Text('Add Another Complaint', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.navy)),
content: const Text('Would you like to add another complaint ticket to this submission?'),
actions: [
TextButton(
onPressed: () => Navigator.pop(context),
...
setState(() {
Map<String, dynamic> aduanBaru = Map<String, dynamic>.from(value);
...
child: const Text('Yes, Add', style: TextStyle(color: AppColors.navy, fontWeight: FontWeight.bold)),
),
],
);
},
  ...
  Future<void> _simpanSemuaAduanKeDatabase() async {
    if (_senaraiAduan.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Your submission list is empty. Please add a complaint.')),
      );
      return;
    }
    ...
    try {
      ...
      requestMultipart.fields['aduan_json_data'] = json.encode(listHantaranTulen);
      ...
      for (int i = 0; i < _senaraiAduan.length; i++) {
        ...
        } else if (aduanSemasa['attachment_file_path'] != null && aduanSemasa['attachment_file_path'].toString().isNotEmpty) {
      ...
      print("📤 MULTIPART STEAM LAUNCHED: Sending ${_senaraiAduan.length} complaints along with physical images tracks...");
      ...
      if (responHasil.statusCode == 200) {
        ...
        if (hasil['status'] == 'berjaya') {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(hasil['mesej'] ?? 'Complaint successfully submitted along with image evidence!')),
            );

            List<String> ticketIds = List<String>.from(hasil['ticket_ids'] ?? []);
        ...
        children: [
          Row(
            children: [
              ...
              TextButton(
                ...
                child: const Text('Edit', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.navy)),
              ),
              const SizedBox(width: 12),
              TextButton(
                onPressed: onCancel,
                style: TextButton.styleFrom(padding: EdgeInsets.zero),
                child: const Text('Cancel', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.redAccent)),
              ),
            ],
          ),
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 16),
              Expanded(child: ReviewField(label: 'Department', value: department)),
            ],
          ),
          const SizedBox(height: 14),
          Row(
            children: [
              Expanded(child: ReviewField(label: 'Complaint Category', value: category)),
              const SizedBox(width: 16),
...
crossAxisAlignment: CrossAxisAlignment.start,
children: [
const Text('Confirm All Submissions', style: AppTextStyles.cardTitle),
...
child: Center(child: Text('No complaints to submit. Please add another item.')),
),
...
fullName: _safeLiveName, // DINAMIK: Mengikut emel login semasa!
email: _safeLiveEmail,   // DINAMIK: Mengikut emel login semasa!
...
attachmentName: aduan['attachment_name'] ?? 'No attachment',
onEdit: () {
Navigator.pushNamed(context, '/complaint/fill', arguments: aduan);
},
...
title: 'Once you click Submit All, all items above will be submitted and cannot be changed.',
),
