...
// Pengurusan Senarai Data
  ...
  List<dynamic> _displayedTickets =
      []; // Menyimpan data selepas ditapis & disususn
...
// --- BAHAGIAN B: TAPISAN BERDASARKAN TEXT CARIAN (SEARCH BOX) ---
    if (kataKunci.isNotEmpty) {
      hasilTapisan = hasilTapisan.where((ticket) {
        ...
        String email =
            (ticket['submitter_email'] ?? '').toString().toLowerCase();
...
// Susun dari tarikh baharu ke lama
      hasilTapisan.sort((a, b) => (b['created_at'] ?? '')
          .toString()
          .compareTo((a['created_at'] ?? '').toString()));
    } else if (_sortBy == 'Oldest') {
// Susun dari tarikh lama ke baharu
      hasilTapisan.sort((a, b) => (a['created_at'] ?? '')
          .toString()
...
// Susun mengikut tahap kritikal: High -> Medium -> Low
      ...
      hasilTapisan.sort((a, b) {
        int weightA = priorityWeight[
                (a['priority'] ?? 'medium').toString().toLowerCase()] ??
            2;
        int weightB = priorityWeight[
                (b['priority'] ?? 'medium').toString().toLowerCase()] ??
            2;
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              style: const TextStyle(
                                  ...
                                  fontWeight: FontWeight.w600),
                              isExpanded: true,
                              items: <String>['Latest', 'Oldest', 'Priority']
                                  .map((String value) {
...
// Memaparkan info jumlah rekod dikira dinamik
                  Text(
                      'Showing ${_displayedTickets.length} of ${_allTickets.length} tickets',
                      style: const TextStyle(
