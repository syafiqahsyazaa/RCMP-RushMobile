...
// Pengurus data dinamik dari database unicomplaint
  Map<String, dynamic> _stats = {"open": "0", "in_progress": "0", "closed": "0", "unresolved": "0"};
...
//MENYIMPAN DATA CARTA DARI DATABASE
  ...
  final List<Color> _chartColors = [const Color(0xFF7C3AED), const Color(0xFF2F5FA3), const Color(0xFF2E9E52)];
...
// Fungsi menarik maklumat statistik & aduan masuk dari Laragon API
  Future<void> _ambilDataStaffDashboard({bool isBackground = false}) async {
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        if (!isBackground) {
          print("Gagal muat data staff dashboard. Status: ${respon.statusCode}");
  ...
  void _prosesTapisTiketDashboard() {
    ...
    if (_tab == 0) {
      ...
      hasilTapis = _rawAllTicketsFromDB.where((ticket) {
        String ticketRemarks = (ticket['remarks'] ?? '').toString().toLowerCase().trim();
      ...
      hasilTapis = _rawAllTicketsFromDB.where((ticket) {
        String ticketRemarks = (ticket['remarks'] ?? '').toString().toLowerCase().trim();
        String priority = (ticket['priority'] ?? '').toString().toLowerCase().trim();
      ...
      hasilTapis = _rawAllTicketsFromDB.where((ticket) {
        ...
        DateTime endTime = status == 'open' ? DateTime.now() : DateTime.parse(ticket['updated_at'] ?? dateStr);
  ...
  Widget build(BuildContext context) {
    ...
    style: const TextStyle(color: AppColors.navy, fontSize: 13, fontWeight: FontWeight.w700),
    ),
    actions: [
    ...
    child: Text(
    _openTickets.where((t) => (t['remarks'] ?? '').toString().toLowerCase().trim() == _namaStaffLive.toLowerCase().trim()).length.toString(),
    ...
    int currentNotificationTab = _stats['notif_tab_active_index'];
    ...
    senaraiNotifikasiYangDahDitapis = senaraiAduanTiketSemasaDB;
    } else if (currentNotificationTab == 1) {
    // 🟢 TAB ASSIGNED TO ME: Hanya menapis aduan tiket Hak Milik yang sedang login!
    senaraiNotifikasiYangDahDitapis = senaraiAduanTiketSemasaDB.where((tkt) {
    String ticketRemarks = (tkt['remarks'] ?? '').toString().toLowerCase().trim();
    return ticketRemarks == _namaStaffLive.toLowerCase().trim();
    }).toList();
    ...
    if (dateStr.isEmpty) return false;
    DateTime createdTime = DateTime.parse(dateStr);
    String status = (tkt['status'] ?? 'open').toString().toLowerCase();
    DateTime endTime = status == 'open' ? DateTime.now() : DateTime.parse(tkt['updated_at'] ?? dateStr);
    ...
    Widget buildLocalNotificationTabButton(String label, bool isBtnActive, VoidCallback onBtnTap) {
    return InkWell(
    onTap: onBtnTap,
    ...
    color: isBtnActive ? const Color(0xFFEAF1FB) : Colors.transparent,
    borderRadius: BorderRadius.circular(20),
    ),
    child: Text(
    label,
    style: TextStyle(
    fontSize: 11,
    fontWeight: isBtnActive ? FontWeight.w900 : FontWeight.w500,
    color: isBtnActive ? const Color(0xFF2F5FA3) : Colors.grey.shade600
    ),
    ),
    ),
    );
    ...
    children: [
    const Text('Notifications', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: AppColors.navy)),
    Container(
    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
    decoration: BoxDecoration(color: const Color(0xFFEAF1FB), borderRadius: BorderRadius.circular(12)),
    child: Text('${senaraiNotifikasiYangDahDitapis.length} active', style: const TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: AppColors.navy)),
    ),
    ],
    ),
    ...
    mainAxisAlignment: MainAxisAlignment.spaceBetween,
    children: [
    buildLocalNotificationTabButton('All', currentNotificationTab == 0, () => setMenuState(() => _stats['notif_tab_active_index'] = 0)),
    buildLocalNotificationTabButton('Assigned to Me', currentNotificationTab == 1, () => setMenuState(() => _stats['notif_tab_active_index'] = 1)),
    buildLocalNotificationTabButton('Deadlines', currentNotificationTab == 2, () => setMenuState(() => _stats['notif_tab_active_index'] = 2)),
    ],
    ),
    ...
    child: Icon(Icons.notifications_off_outlined, size: 26, color: Colors.grey.shade300),
    ),
    const SizedBox(height: 10),
    const Text('No notifications', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
    const SizedBox(height: 2),
    const Text("You're all caught up!", style: TextStyle(fontSize: 10, color: Colors.grey)),
    ],
    ),
    ),
    )
    ...
    itemCount: senaraiNotifikasiYangDahDitapis.length,

    separatorBuilder: (context, index) => const SizedBox(height: 8),
    itemBuilder: (context, index) {
    final tiket = senaraiNotifikasiYangDahDitapis[index];
// Semak baki waktu bekerja SLA tiket ini secara real-time!
    String dateStr = tiket['created_at'] ?? '';
    DateTime createdTime = DateTime.parse(dateStr);
    String currentStatus = (tiket['status'] ?? 'open').toString().toLowerCase();
    DateTime endTime = currentStatus == 'open' ? DateTime.now() : DateTime.parse(tiket['updated_at'] ?? dateStr);
    bool adakahTiketIniBreachedSlaLive = _kiraMinitBekerjaSLA(createdTime, endTime) > 480;
...
// Terus membawa petugas masuk ke skrin workspace detail tiket!
    final muatSemulaDashboardStaff = await Navigator.pushNamed(
    context,
    '/tickets/workspace',
    arguments: tiket
    );
    if (muatSemulaDashboardStaff == true) {
    _ambilDataStaffDashboard();
    }
    },
    child: Container(
    padding: const EdgeInsets.all(10),
    decoration: BoxDecoration(
    color: adakahTiketIniBreachedSlaLive ? const Color(0xFFFDF2F2) : const Color(0xFFF3F7FC),
    borderRadius: BorderRadius.circular(12),
    ),
    child: Row(
    crossAxisAlignment: CrossAxisAlignment.start,
    ...
    CircleAvatar(
    radius: 15,
    backgroundColor: adakahTiketIniBreachedSlaLive ? const Color(0xFFD64545) : const Color(0xFF3B82F6),
    child: Text(adakahTiketIniBreachedSlaLive ? '⚠️' : 'NT', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
    ),
    ...
    mainAxisAlignment: MainAxisAlignment.spaceBetween,
    children: [
    Expanded(child: Text(tiket['title'] ?? 'Complaint Ticket', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.textPrimary), overflow: TextOverflow.ellipsis)),
    Text(
    adakahTiketIniBreachedSlaLive ? 'BREACHED •' : '',
    style: TextStyle(fontSize: 8.5, fontWeight: FontWeight.bold, color: adakahTiketIniBreachedSlaLive ? const Color(0xFFD64545) : Colors.blue)
    ),
    ],
    ),
    const SizedBox(height: 2),
    Text(tiket['ticket_id'] ?? '', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.textSecondary)),
    const SizedBox(height: 2),
    Text(
    ...
    Text('${tiket['my_department'] ?? "IT Dept"} · ${tiket['created_at']}', style: const TextStyle(fontSize: 9, color: Colors.grey)),
    ],
    ),
    )
    ],
    ...
    body: SafeArea(
        child: _isLoading
            ...
            : RefreshIndicator(
          onRefresh: () => _ambilDataStaffDashboard(), // Tarik data baru apabila skrin di-pull down
          child: ListView(
...
// Nilai kad di bawah kini membaca dari database Laragon secara langsung
                    StatMiniCard(value: _stats['open']!, label: 'Open Tickets', icon: Icons.mark_email_unread_outlined, color: const Color(0xFF2F5FA3)),
                    const SizedBox(width: 10),
                    StatMiniCard(value: _stats['in_progress']!, label: 'In Progress', icon: Icons.autorenew, color: const Color(0xFFC9A227)),
                    const SizedBox(width: 10),
                    StatMiniCard(value: _stats['closed']!, label: 'Recently Closed', icon: Icons.check_circle_outline, color: const Color(0xFF2E9E52)),
                    const SizedBox(width: 10),
                    StatMiniCard(value: _stats['unresolved']!, label: 'Unresolved', icon: Icons.error_outline, color: const Color(0xFFD64545)),
                  ],
                ),
              ),
...
// JIKA DATA CARTA MASIH KOSONG DI DATABASE
                    _chartData.isEmpty
                      ...
                      child: Text('No complaint data available for chart analysis.', style: TextStyle(fontSize: 11, color: AppColors.textMuted)),
                    )
                        : Column(
                      children: [
                        //  JANA KANDUNGAN SLICES SECARA LIVE BERDASARKAN HASIL SQL
                        SimpleDonutChart(
                          slices: _chartData.map<MapEntry<String, double>>((item) {
                            String namaDept = (item['name'] ?? 'General').toString();
                            double jumlahDouble = double.parse((item['total'] ?? 0).toString());
                            return MapEntry<String, double>(namaDept, jumlahDouble);
                          }).toList(),
                          colors: _chartColors.take(_chartData.length).toList(),
                        ),
                        const SizedBox(height: 12),

                        // Penunjuk warna (Legend)
                        ChartLegend(
                          items: List<MapEntry<String, Color>>.generate(_chartData.length, (index) {
                            String namaDept = (_chartData[index]['name'] ?? 'General').toString();
                            return MapEntry<String, Color>(namaDept, _chartColors[index]);
                          }),
                        ),
                      ],
                    ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _displayedTickets.isEmpty
                        ? const Padding(
                      padding: EdgeInsets.symmetric(vertical: 20),
                      child: Center(child: Text('No active records under this profile track.', style: TextStyle(fontSize: 11, color: AppColors.textMuted))),
                    )
                        : Column(
                      children: _displayedTickets.map((ticket) {
                        String idTiket = ticket['ticket_id'] ?? 'No ID';
                        String katName = ticket['category'] ?? ticket['title'] ?? 'General';
                        String deptName = ticket['my_department'] ?? 'No Dept';
                        String rawStatus = (ticket['status'] ?? 'open').toString().toUpperCase().replaceAll('_', ' ');
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Text(idTiket, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.bold, color: AppColors.navy)),
                                  Text(rawStatus, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: _getStatusColor(ticket['status'] ?? 'open'))),
                                ],
                              ),
                              const SizedBox(height: 6),
                              Text("Category: $katName", style: const TextStyle(fontSize: 11, color: AppColors.textPrimary)),
                              const SizedBox(height: 2),
                              Text("From: $deptName", style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                              ...
                              Align(
                                ...
                                child: InkWell(
                                  onTap: () async {
                                    final muatSemula = await Navigator.pushNamed(context, '/tickets/workspace', arguments: ticket);
                                    if (muatSemula == true) _ambilDataStaffDashboard();
                                  },
                                  child: const Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      Text("View Details", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.navy)),
                                      SizedBox(width: 4),
                                      Icon(Icons.arrow_forward_rounded, size: 12, color: AppColors.navy),
                                    ],
                                  ),
                                ),
                              )
