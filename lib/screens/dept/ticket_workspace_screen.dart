import 'package:flutter/material.dart';import 'package:flutter/foundation.dart';import 'package:http/http.dart' as http;import 'dart:convert';import '../../theme/app_theme.dart';import '../../theme/admin_theme.dart';import '../../main.dart';
  ...
  void didChangeDependencies() {
    ...
    if (!_isInitialized) {
      ...
      if (args != null && args is Map<String, dynamic>) {
        ...
        _namaStaffLive = (args['full_name'] ?? args['name'] ?? 'name').toString().trim();
        ...
        if (args['assigned_vendor_id'] != null && args['assigned_vendor_id'].toString() != '0' && args['assigned_vendor_id'].toString() != 'null') {
          _isAssignedToVendorLive = true;
          _selectedAssignedEntityValue = "vendor${args['assigned_vendor_id']}".toString().trim();
          _assignedVendorNameLive = args['assigned_vendor_name'] ?? args['remarks'] ?? 'Vendor Account';
        } else if (savedStaffName.toLowerCase().startsWith('vendor') || savedStaffName.toLowerCase().contains('sdn') || savedStaffName.toLowerCase().contains('bhd') || savedStaffName.toLowerCase().contains('snd')) {
          _isAssignedToVendorLive = true;
          _assignedVendorNameLive = args['assigned_vendor_name'] ?? savedStaffName;
          _selectedAssignedEntityValue = savedStaffName;
        }

        String? dbUpdatedAt = args['updated_at'];
        if (dbUpdatedAt != null && dbUpdatedAt.isNotEmpty && dbUpdatedAt != "null") {
...
// =========================================================================
  ...
  final url = Uri.parse('http://$domain/helpdesk_api/get_user.php?email=$currentLoggedInUserEmail');
  try {
    ...
    if (respon.statusCode == 200) {
      ...
      if (hasil['status'] == 'wujud') {
        setState(() {
          _namaStaffLive = (hasil['data']['full_name'] ?? _namaStaffLive).toString().trim();
          _jabatanStaffSesiLive = (hasil['data']['department_name'] ?? hasil['data']['department'] ?? "").toString().trim();
        });
        print("🔎 SESSION IDENTIFIED: User $_namaStaffLive from $_jabatanStaffSesiLive");
        _tarikActiveVendorsDropdownDariPHP();
      }
    }
  } catch (e) { print("Error fetch name: $e"); }
}
...
// =========================================================================
      ...
      if (respon.statusCode == 200) {
        ...
        if (freshData != null) {
          setState(() {
            ...
            String dbStatusRaw = (_ticketData['status'] ?? 'open').toString().toLowerCase().trim();
            ...
            String remarksText = (_ticketData['remarks'] ?? '').toString().toLowerCase().trim();
            String vendorNameText = (_ticketData['assigned_vendor_name'] ?? '').toString().toLowerCase().trim();

            // Jika dikesan mengandungi data vendor, KOTAK KUNING WAJIB NYALA TRUE!
            if ((vId != '0' && vId != 'null') ||
                remarksText.contains('vendor') || remarksText.contains('sdn') || remarksText.contains('bhd') || remarksText.contains('itdeptvendor') ||
                vendorNameText.contains('vendor') || vendorNameText.contains('sdn') || vendorNameText.contains('bhd') || vendorNameText.contains('itdeptvendor')) {

              // Paksa gembok status menjadi TRUE tanpa kompromi!
              _isAssignedToVendorLive = true;

              String namaSyarikatMentah = _ticketData['assigned_vendor_name'] ?? _ticketData['remarks'] ?? 'ITDEPTVENDOR';
              ...
              if (_senaraiActiveVendorsDropdown.isNotEmpty) {
                final matchDropdown = _senaraiActiveVendorsDropdown.firstWhere(
                      (v) => (v['company_name'] ?? '').toString().trim().toLowerCase() == _assignedVendorNameLive.toLowerCase(),
                  orElse: () => null,
                );
                if (matchDropdown != null) {
                  _selectedAssignedEntityValue = matchDropdown['company_name'].toString().trim();
                }
              }
            } else {
          ...
          print("✅ STAFF-ADMIN SHARE WORKSPACE DATA LOCKED: Vendor Box Status = $_isAssignedToVendorLive");
        }
      }
    } catch (e) {
  ...
  Future<void> _tarikActiveVendorsDropdownDariPHP() async {
    ...
    if (lowerDept.contains('administration') || lowerDept.contains('facilities') || lowerDept.contains('afsmd')) {
      finalCleanDeptKeyword = "Administration & Facilities Management Department";
    } else if (lowerDept.contains('maintenance')) {
      finalCleanDeptKeyword = "Maintenance Department";
    } else if (lowerDept.contains('it') || lowerDept.contains('information technology') || lowerDept.contains('dept/')) {
      finalCleanDeptKeyword = "Information Technology Department";
    } else if (lowerDept.contains('human capital') || lowerDept.contains('hcd')) {
      finalCleanDeptKeyword = "Human Capital Department";
    } else if (lowerDept.contains('corporate communication') || lowerDept.contains('ccu')) {
      finalCleanDeptKeyword = "Corporate Communication Unit";
    }

    print("🎯 DYNAMIC TARGET ENJIN DROPDOWN LOCKED FOR DEPT: [$finalCleanDeptKeyword] (Complaints Dept ID: $ticketDeptId)");

    final url = Uri.parse('http://$domain/helpdesk_api/get_active_vendors_dropdown.php?department=${Uri.encodeComponent(finalCleanDeptKeyword)}&b_cache=${DateTime.now().millisecondsSinceEpoch}');
    ...
    try {
      ...
      if (respon.statusCode == 200) {
        ...
        print("SUCCESS DISPATCH DROPDOWN LOADED: ${_senaraiActiveVendorsDropdown.length} vendor items allocated for [$finalCleanDeptKeyword]");
      }
    } catch (e) {
  ...
  try {
    ...
    if (respon.statusCode == 200) {
      ...
      setState(() { _ticketLogs = data; });
    }
  } catch (e) { print("Ralat menarik sejarah aktiviti tiket: $e"); }
}
...
Future<void> _ambilFeedbackTiket(String idTiket) async {
  ...
  Future<void> _ambilDataStaff() async {
    ...
    final url = Uri.parse('http://$domain/helpdesk_api/get_staff_members.php?email=$emelStaffSemasa&ticket_id=$_currentTicketId&b_cache=${DateTime.now().millisecondsSinceEpoch}');

    try {
      final respon = await http.get(url);
      if (respon.statusCode == 200) {
        final List<dynamic> data = json.decode(respon.body);
        setState(() { _listStaff = List<String>.from(data); });
      }
    } catch (e) { print("Ralat mengambil nama staff: $e"); }
  }
...
setState(() { _slaData = json.decode(respon.body); });
...
} catch (e) { print("Ralat mengambil data SLA: $e"); }
}
  ...
  Future<void> _hantarKemasKiniStatusKePHP({
    ...
    final String domain = kIsWeb ? 'localhost' : '10.103.19.67';
    final url = Uri.parse('http://$domain/helpdesk_api/update_ticket_status.php');
    ...
    if (_selectedAssignedEntityValue != null) {
      ...
      if (_senaraiActiveVendorsDropdown.isNotEmpty) {
        final padananVendorDariNamaSyarikat = _senaraiActiveVendorsDropdown.firstWhere(
              (v) => (v['company_name'] ?? '').toString().trim().toLowerCase() == entitiDipilihLive.toLowerCase(),
          orElse: () => null,
        );

        if (padananVendorDariNamaSyarikat != null) {
          //  ambil token ID nombor vendor tulin dari database
          finalVendorId = (padananVendorDariNamaSyarikat['vendor_id'] ?? padananVendorDariNamaSyarikat['id'] ?? '0').toString().trim();

          // Setkan data string format simpanan text database complaints
          targetStaffNameToSave = entitiDipilihLive; // Menyimpan string nama "SYA and bhd" ke remarks/staff
        }
      }
    }
    // =========================================================================
    ...
    try {
      final respon = await http.post(
        ...
        body: json.encode({
          ...
          "vendor_id": finalVendorId, // Kirim id nombor yang sah ke PHP Laragon!
        }),
      );
...
setState(() {
_lastUpdatedText = hasil['updated_at'] ?? DateTime.now().toString();
...
_ambilFeedbackTiket(_currentTicketId);
...
} else { _tampilMesej(hasil['mesej'] ?? "Gagal menyimpan ."); }
}
} catch (e) { _tampilMesej("Ralat Sambungan Laragon: $e"); }
  ...
  void _semakSyaratSebelumSimpanNormal() {
    String targetStaff = _selectedAssignedEntityValue != null && !_selectedAssignedEntityValue.toString().startsWith("vendor")
        ? _selectedAssignedEntityValue.toString()
        : (_namaStaffLive.isNotEmpty ? _namaStaffLive : 'Help Desk Support');

    String statusAsalDatabase = (_ticketData['status'] ?? 'open').toString().toLowerCase().trim();
    ...
    String assigneeBaruPilihan = (_selectedAssignedEntityValue ?? '').toString().trim();
    ...
    if (adakahStatusBerubah || adakahAssigneeBerubah) {
      ...
      String hintDialog = 'Enter status update note (e.g., Attending to site issue)...';
...
required String tajuk, required String hint, required Color warnaButang, required Function(String) onConfirm, VoidCallback? onCancel
}) {
showDialog(
context: context,
...
void _prosesSimpanTerusDariStatus(String statusPilihan) {
  ...
  setState(() { _selectedStatus = statusPilihan; });
  ...
  _pamerDialogRemarksPaksaan(
      ...
      onCancel: () {
        ...
        setState(() { _selectedStatus = statusLamaSesi; });
      }
  );
}
...
void _prosesSimpanTerusDariReassign(dynamic newValue) {
  ...
  _pamerDialogRemarksPaksaan(
      ...
      onCancel: () {
         setState(() { _selectedAssignedEntityValue = valueLamaSesi; });
      }
  );
}
void _tampilMesej(String mesej) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(mesej))); }
...
bool isStatusInProgressLive = (_selectedStatus?.toLowerCase().replaceAll(' ', '') == 'in_progress');
...
String currentStatus = (_selectedStatus ?? _ticketData['status'] ?? 'open').toString().toLowerCase();
...
SizedBox(width: 8),
Text('Feedback tab is LOCKED until the ticket is officially CLOSED!', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
],
),
...
style: TextStyle(color: AppColors.navy, fontSize: 13, fontWeight: FontWeight.w700),
),
),
body: SafeArea(
child: Column(
...
TabBar(
labelColor: AppColors.navy, unselectedLabelColor: AppColors.textSecondary,
...
children: [
Text((_selectedStatus ?? _ticketData['status'] ?? 'open').toString().toLowerCase() == 'closed' ? 'Feedback 💬' : 'Feedback 🔒'),
],
),
),
],
...
child: Text(description, style: const TextStyle(fontSize: 12.5, color: AppColors.textPrimary, height: 1.4)),
  ...
  if (_ticketData['attachment_path'] != null && _ticketData['attachment_path'].toString().isNotEmpty && _ticketData['attachment_path'].toString() != 'null') ...[
    const SizedBox(height: 16),
    const Text('Attachment Media', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
    const SizedBox(height: 8),
    InkWell(
      onTap: () {
        final String domain = kIsWeb ? 'localhost' : '10.103.19.67';
        ...
        String namaFailMentahDariDb = _ticketData['attachment_path'].toString().trim();
        String namaFailSiapEncode = Uri.encodeComponent(namaFailMentahDariDb);

        // Gabungkan url kacak yang dibilas suci bersih total 100%!
        String urlGambarTulen = "http://$domain/helpdesk_api/uploads/$namaFailSiapEncode";
        ...
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            title: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('📸 Evidence View', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.navy)),
                      ...
                      child: ClipRRect(
                        ...
                        child: Image.network(
                          ...
                          errorBuilder: (context, error, stackTrace) => const Padding(
                            ...
                            child: Text('❌ Image file not found inside Laragon uploads folder.', style: TextStyle(fontSize: 11, color: Colors.red)),
                          ),
                        ),
                      ),
                    ),
                    const SizedBox(height: 10),
                    Text(namaFailMentahDariDb, style: const TextStyle(fontSize: 11, color: Colors.grey)),
                  ],
                ),
              ),
            ),
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(color: const Color(0xFF2F5FA3).withOpacity(0.3))
        ),
        child: Row(
          children: [
            const Icon(Icons.image_outlined, size: 18, color: AppColors.navy),
            ...
            const Icon(Icons.open_in_new_rounded, size: 14, color: AppColors.navy),
          ],
        ),
      ),
    ),
...
const SizedBox(height: 16),
const Text('Submitted By', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
const SizedBox(height: 8),
Container(
width: double.infinity, padding: const EdgeInsets.all(14),
decoration: BoxDecoration(color: AppColors.fieldBackground, borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.border.withValues(alpha: 0.5))),
child: Row(children: [Expanded(child: Text(emailUser, style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: AppColors.textPrimary)))]),
),
],
),
),
    ...
    child: Column(
      ...
      children: [
        Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: const Color(0xFFEAF7EE), borderRadius: BorderRadius.circular(6)),
              child: const Icon(Icons.access_time, size: 16, color: Color(0xFF2E9E52)),
            ),
            const SizedBox(width: 10),
            const Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('SLA Status', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                Text('8 hrs · Mon–Fri 08:00–17:00', style: TextStyle(fontSize: 10, color: AppColors.textMuted)),
              ],
            ),
          ],
        ),
        ...
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
          decoration: BoxDecoration(
              color: (_slaData?['is_breached'] ?? false) ? const Color(0xFFFDF2F2) : const Color(0xFFEAF7EE),
            ...
            children: [
              Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      color: (_slaData?['is_breached'] ?? false) ? const Color(0xFFD64545) : const Color(0xFF2E9E52)
                  )
              ),
              const SizedBox(width: 6),
              Text(
                  _isAssignedToVendorLive
                      ? (_slaData?['admin_sla_status']?.toString() ?? "SLA Stopped")
                      : (_slaData?['sla_status']?.toString() ?? "Active"),
                  style: TextStyle(
                      fontSize: 10.5,
        ...
        LinearProgressIndicator(
          value: double.parse((_isAssignedToVendorLive ? (_slaData?['admin_progress'] ?? 0.0) : (_slaData?['progress'] ?? 0.0)).toString()),
          backgroundColor: AppColors.pageBackground,
          valueColor: AlwaysStoppedAnimation((_slaData?['is_breached'] ?? false) ? const Color(0xFFD64545) : const Color(0xFF2E9E52)),
          minHeight: 6, borderRadius: BorderRadius.circular(4),
        ),
        const SizedBox(height: 4),
        const Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [Text('0h', style: TextStyle(fontSize: 10, color: AppColors.textMuted)), Text('4h', style: TextStyle(fontSize: 10, color: AppColors.textMuted)), Text('8h', style: TextStyle(fontSize: 10, color: AppColors.textMuted))],
        ),
        const SizedBox(height: 16),
        SingleChildScrollView(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildSlaInfoColumn("SUBMITTED", _slaData?['submitted_at']?.toString() ?? '—', "Ticket submission\ndate & time"),
              _buildSlaInfoSpacer(),
              _buildSlaInfoColumn("DEADLINE", "—", "SLA clock stopped\nafter response"),
              _buildSlaInfoSpacer(),
              _buildSlaInfoColumn(
                  "TIME USED",
                  _isAssignedToVendorLive ? (_slaData?['admin_time_used']?.toString() ?? "0h m used") : (_slaData?['time_used']?.toString() ?? "—"),
                  "Working hours from\nsubmission to close",
                  isHighlight: true
              ),
              _buildSlaInfoSpacer(),
              _buildSlaInfoColumn("RESPONDED AT", _slaData?['responded_at']?.toString() ?? '—', "Staff first moved\nticket to In Progress"),
              _buildSlaInfoSpacer(),
              _buildSlaInfoColumn("CLOSED AT", _slaData?['closed_at']?.toString() ?? '—', "Ticket was marked as\nclosed"),
            ],
          ),
        )
      ],
...
Row(
children: const [
Icon(Icons.person_outline_outlined, color: AppColors.navy, size: 16),
SizedBox(width: 8),
Text('Assigned To', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
],
),
const SizedBox(height: 4),
const Text('Manage ticket assignment parameters', style: TextStyle(fontSize: 10, color: Colors.grey)),
const Divider(height: 20, color: AppColors.border),
// Kotak Paparan Penanda Status Atas Live
_isAssignedToVendorLive
? Container(
width: double.infinity, padding: const EdgeInsets.all(12),
decoration: BoxDecoration(color: const Color(0xFFEAF1FB), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFFD2E3FC))),
child: Row(
children: [
CircleAvatar(radius: 12, backgroundColor: Colors.purple.shade50, child: const Text('V', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.purple))),
const SizedBox(width: 10),
Expanded(
child: Row(
mainAxisAlignment: MainAxisAlignment.spaceBetween,
children: [
Text(
(_ticketData['assigned_vendor_name'] != null && _ticketData['assigned_vendor_name'].toString().isNotEmpty && _ticketData['assigned_vendor_name'].toString() != 'null')
? _ticketData['assigned_vendor_name'].toString().toUpperCase()
    : (_ticketData['remarks'] != null && _ticketData['remarks'].toString().isNotEmpty)
? _ticketData['remarks'].toString().toUpperCase()
    : (_assignedVendorNameLive.isNotEmpty ? _assignedVendorNameLive.toUpperCase() : 'VENDOR COMPANY'),
style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.navy)
),
Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(4)), child: const Text('Vendor', style: TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: Colors.blue))),
],
),
)
],
...
child: Text(_selectedStaff != null && _selectedStaff!.isNotEmpty ? _selectedStaff!.substring(0, 1).toUpperCase() : 'H', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700))
),
const SizedBox(width: 10),
Expanded(
child: Text(
(_selectedStaff ?? 'Help Desk Support').toString().toUpperCase(),
style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: AppColors.textPrimary)
  ...
  if (_isAssignedToVendorLive == true) ...[
    (() {
      String jamDanMinitSlaLiveDipapar = "5h 0m left";
      String dueTimeLiveTulenDipapar = "Pending Calculation...";
      ...
      try {
        ...
        if (_ticketLogs.isNotEmpty) {
          final logVendor = _ticketLogs.firstWhere(
            (log) => (log['reason'] ?? '').toString().toUpperCase().contains('ASSIGNED TO') ||
                     (log['reason'] ?? '').toString().toUpperCase().contains('VENDOR'),
            orElse: () => null,
          );
          if (logVendor != null) waktuMulaSlaStr = (logVendor['transferred_at'] ?? '').toString();
        }

        if (waktuMulaSlaStr.isEmpty) {
          waktuMulaSlaStr = (_ticketData['updated_at'] ?? _ticketData['created_at'] ?? '').toString();
        }

        // !
        DateTime? waktuMula;
        if (waktuMulaSlaStr.isNotEmpty) {
           waktuMula = DateTime.tryParse(waktuMulaSlaStr);
        }

        //
        waktuMula ??= DateTime.tryParse(_ticketData['created_at'] ?? '') ?? DateTime.now();

        // PENGIRAAN DEADLINE 5 JAM KERJA VENDOR
        DateTime deadline = waktuMula.add(const Duration(hours: 5));
        
        // Format paparan Due Time
        List<String> months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        String amPm = deadline.hour >= 12 ? 'PM' : 'AM';
        int displayHour = deadline.hour > 12 ? deadline.hour - 12 : (deadline.hour == 0 ? 12 : deadline.hour);
        dueTimeLiveTulenDipapar = "${deadline.day} ${months[deadline.month - 1]}, $displayHour:${deadline.minute.toString().padLeft(2, '0')} $amPm";

        // Kira baki masa
        Duration diff = deadline.difference(DateTime.now());
        if (diff.isNegative) {
          jamDanMinitSlaLiveDipapar = "0h 0m left (BREACHED)";
        } else {
          jamDanMinitSlaLiveDipapar = "${diff.inHours}h ${diff.inMinutes % 60}m left";
        }
        
      } catch (e) {
        dueTimeLiveTulenDipapar = "03 Sep, 10:21 AM";
      }

      String statusTulinDbComplaints = (_ticketData['status'] ?? _selectedStatus ?? 'open').toString().toLowerCase().trim();

      // filter VENDOR: Hanya log yang mengandungi 'HANDLED BY: 👤' dikira sebagai respon vendor
      bool adakahVendorDahResponLive = _ticketLogs.any((log) {
        String reasonLog = (log['reason'] ?? '').toString().toUpperCase();
      ...
      if (statusTulinDbComplaints == 'closed' || adakahVendorDahResponLive) {
        return Container(
          width: double.infinity, margin: const EdgeInsets.only(top: 12),
          padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(color: const Color(0xFFEAF7EE), borderRadius: BorderRadius.circular(10), border: Border.all(color: const Color(0xFF2E9E52).withOpacity(0.25))),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('VENDOR RESPONSE', style: TextStyle(fontSize: 9.5, color: Colors.grey, letterSpacing: 0.5)),
              const SizedBox(height: 8),
              Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFD1FAE5), borderRadius: BorderRadius.circular(4)), child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.check_circle, size: 12, color: Color(0xFF2E9E52)), SizedBox(width: 4), Text('Vendor responded', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF2E9E52)))])),
              const SizedBox(height: 10),
              ...
              Text('Sent to vendor ${_slaData?['responded_at'] ?? _ticketData['updated_at'] ?? '02 Sep 2026, 2:20 PM'} · 5 working hours to respond', style: const TextStyle(fontSize: 10, color: Colors.grey)),
            ],
          ),
        );
      } else {
        ...
        return Container(
          width: double.infinity, margin: const EdgeInsets.only(top: 12),
          padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(color: const Color(0xFFFFF8E6), borderRadius: BorderRadius.circular(10), border: Border.all(color: const Color(0xFFC9A227).withOpacity(0.25))),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('VENDOR RESPONSE', style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 0.5)),
              const SizedBox(height: 8),
              Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: const BoxDecoration(color: Color(0xFFFFF1D2), borderRadius: BorderRadius.all(Radius.circular(4))), child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.hourglass_empty_rounded, size: 12, color: Color(0xFFC9A227)), SizedBox(width: 4), Text('Awaiting vendor response', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFFC9A227)))])),
              const SizedBox(height: 10),
              Text("$jamDanMinitSlaLiveDipapar · due $dueTimeLiveTulenDipapar", style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
              const SizedBox(height: 3),
              Text('Sent to vendor ${_slaData?['responded_at'] ?? _ticketData['updated_at'] ?? '02 Sep 2026, 2:20 PM'} · 5 working hours to respond', style: const TextStyle(fontSize: 10, color: Colors.grey)),
            ],
          ),
  ...
  const SizedBox(height: 12),
const Text('REASSIGN TO', style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.bold, color: Colors.grey)),
...
// =========================================================================
  DropdownButtonFormField(
    ...
    value: (() {
      if (_selectedAssignedEntityValue != null) {
        String strValueLive = _selectedAssignedEntityValue.toString().trim();

        //  Jika awal-awal masuk, takungan entiti memegang id hantu 'vendor18' atau 'vendorXX'
        if (strValueLive.startsWith('vendor') || strValueLive.startsWith('vendors')) {
          // ambil nombor id sahaja
          String bersihkanIdAngka = strValueLive.replaceAll(RegExp(r'[^0-9]'), '').trim();

          if (_senaraiActiveVendorsDropdown.isNotEmpty && bersihkanIdAngka.isNotEmpty) {
            // Cari mana-mana vendor di dalam list dropdown
            final padananVendor = _senaraiActiveVendorsDropdown.firstWhere(
                  (v) => (v['vendor_id'] ?? v['id'] ?? '').toString().trim() == bersihkanIdAngka,
              orElse: () => null,
            );
            if (padananVendor != null) {
              // return nama syarikat
              return (padananVendor['company_name'] ?? '').toString().trim();
            }
          }
        }
        ...
        final bool adakahNamaCompanyWujudDalamDropdown = _senaraiActiveVendorsDropdown.any(
                (v) => (v['company_name'] ?? '').toString().trim().toLowerCase() == strValueLive.toLowerCase()
        );
        if (adakahNamaCompanyWujudDalamDropdown) {
          // Cari ejaan case-sensitive yang tepat di dalam dropdown list  anda
          final exactVendor = _senaraiActiveVendorsDropdown.firstWhere(
                  (v) => (v['company_name'] ?? '').toString().trim().toLowerCase() == strValueLive.toLowerCase()
          );
          return (exactVendor['company_name'] ?? strValueLive).toString().trim();
        }
      }
    ...
    hint: const Text('— Select staff or vendor —', style: TextStyle(fontSize: 12)),
    decoration: InputDecoration(contentPadding: const EdgeInsets.symmetric(horizontal: 10), border: OutlineInputBorder(borderRadius: BorderRadius.circular(6))),

  items: <DropdownMenuItem>[
..._listStaff.map<DropdownMenuItem>((String staffName) {
return DropdownMenuItem(
value: staffName,
child: Text("🧑‍💻 Staff: $staffName", style: const TextStyle(fontSize: 12, color: AppColors.textPrimary)),
);
}),
//  UNLOCK VENDOR (Point 2): Hanya boleh pilih vendor jika status dalam DATABASE sudah IN PROGRESS!
  ..._senaraiActiveVendorsDropdown.map<DropdownMenuItem>((dynamic vendor) {
    String currentDbStatus = (_ticketData['status'] ?? '').toString().toLowerCase().trim();
    bool adakahStatusSudahProgressInDB = currentDbStatus == 'in_progress';

    return DropdownMenuItem(
      value: "${vendor['company_name']}".toString().trim(),
      enabled: adakahStatusSudahProgressInDB, // Hanya enabled jika dah confirm & save status In Progress
      child: Text(
        "🏢 Vendor: ${vendor['company_name']}",
        style: TextStyle(
            fontSize: 11.5,
            color: adakahStatusSudahProgressInDB ? Colors.purple : Colors.grey.shade400,
            fontWeight: FontWeight.bold
        ),
        overflow: TextOverflow.ellipsis,
      ),
...
onChanged: (_selectedStatus?.toLowerCase() != 'closed')
? (dynamic newValue) => _prosesSimpanTerusDariReassign(newValue)
: null,
),
],
),
...
// Kotak Suis Utama Tukar Status Tiket
  Container(
    ...
    child: Builder(
        builder: (context) {
          String staffOwnerDatabase = (_selectedStaff ?? '').toString().toLowerCase().trim();
          String staffLoginSesiTulen = (_namaStaffLive).toString().toLowerCase().trim();

            bool isTicketMineLive = (staffOwnerDatabase == staffLoginSesiTulen) ||
              (staffOwnerDatabase == 'help desk support') ||
              (staffOwnerDatabase == 'unassigned');

          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Update Ticket Status & Priority', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
              const SizedBox(height: 14),
              const Text('PRIORITY', style: AppTextStyles.fieldLabel),
              const SizedBox(height: 8),
              Row(
                children: [
                    _PriorityButton(
                      ...
                      isTicketMine: _selectedStatus?.toLowerCase() != 'closed', // 💥 Locked interaction if closed!
                  ...
                  _PriorityButton(
                      ...
                      active: _selectedPriority == 'Medium',
                      isTicketMine: _selectedStatus?.toLowerCase() != 'closed', // 💥 Locked interaction if closed!
              ...
              const Text('STATUS', style: AppTextStyles.fieldLabel),
              const SizedBox(height: 8),
              Row(
                children: [
                  _StatusButton(
                    ...
                    active: _selectedStatus == 'Open',
                     isTicketMine: isTicketMineLive && (_ticketData['status']?.toString().toLowerCase() != 'closed') && (_selectedStatus != 'Closed'),
                    onTap: () => _prosesSimpanTerusDariStatus('Open'),
                  ),
                  const SizedBox(width: 8),
                  _StatusButton(
                    label: 'In Progress',
                    color: const Color(0xFFC9A227),
                    active: _selectedStatus == 'In Progress',
                    //  Jika tiket dah CLOSED, button In Progress terus locked !
                    isTicketMine: isTicketMineLive && (_ticketData['status']?.toString().toLowerCase() != 'closed') && (_selectedStatus != 'Closed'),
                    onTap: () => _prosesSimpanTerusDariStatus('In Progress'),
                  ),
                  const SizedBox(width: 8),
                  _StatusButton(
                    label: 'Closed',
                    ...
                    isTicketMine: isTicketMineLive && (_ticketData['status']?.toString().toLowerCase() != 'closed'),
                    onTap: () => _prosesSimpanTerusDariStatus('Closed'),
                  ),
                ],
              ),
              const SizedBox(height: 18),
              ...
              else if (_ticketData['status']?.toString().toLowerCase() == 'closed') ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      border: Border.all(color: const Color(0xFF2E9E52).withOpacity(0.3))
                  ),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: const [
                      Icon(Icons.verified_user_rounded, color: Color(0xFF2E9E52), size: 16),
                      SizedBox(width: 8),
                      Text(
                        'This ticket has been officially resolved and CLOSED.',
                        style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.bold, color: Color(0xFF2E9E52)),
                      ),
                    ],
                  ),
                ),
...
// =========================================================================
  _ticketLogs.isEmpty
    ...
    children: List.generate(_ticketLogs.length, (index) {
      ...
      String deskripsiMentah = (log['reason'] ?? 'Activity logged.').toString();
      String tarikhLogMentah = (log['transferred_at'] ?? '').toString();
      bool isStatusChange = deskripsiMentah.toLowerCase().contains('status');

      // STEP 1: FORMAT TARIKH PROFESIONAL (Cth: 11:21 | 18 Sep 2026)
      String tarikhMasaSiapDipapar = tarikhLogMentah;
      if (tarikhLogMentah.length >= 19) {
        try {
          String jamMinit = tarikhLogMentah.substring(11, 16);
          String hari = tarikhLogMentah.substring(8, 10);
          String bulanNum = tarikhLogMentah.substring(5, 7);
          String tahun = tarikhLogMentah.substring(0, 4);

          if (hari.startsWith('0')) hari = hari.substring(1);
          ...
          int indeksBulan = int.parse(bulanNum) - 1;
          String namaBulanTeks = (indeksBulan >= 0 && indeksBulan < 12) ? senaraiBulanTulin[indeksBulan] : 'Sep';

          tarikhMasaSiapDipapar = "$jamMinit | $hari $namaBulanTeks $tahun";
        } catch (e) {
          tarikhMasaSiapDipapar = tarikhLogMentah;
        }
      }

      // STEP 2: LITAR PENYELARAS DESKRIPSI
      String deskripsiBersihDipapar = deskripsiMentah;

      if (deskripsiBersihDipapar.contains('->') || deskripsiBersihDipapar.contains('→')) {
        try {
          if (deskripsiBersihDipapar.toLowerCase().contains('vendor')) {
            String namaSyarikatVendorTulen = (
                _ticketData['assigned_vendor_name'] ??
                    _ticketData['remarks'] ??
                    'itdeptvendor sdn bhd'
            ).toString().trim();

            if (namaSyarikatVendorTulen.isEmpty || namaSyarikatVendorTulen.toLowerCase() == 'null') {
              namaSyarikatVendorTulen = "itdeptvendor sdn bhd";
            }

           if (deskripsiBersihDipapar.contains('→')) {
              String bahagianDepan = deskripsiBersihDipapar.split('→').first;
               if (deskripsiMentah.contains('MESSAGE:')) {
                deskripsiBersihDipapar = deskripsiMentah; // Kekalkan isi penuh Handled By & Message tulen!
              } else {
                deskripsiBersihDipapar = "$bahagianDepan→ ${namaSyarikatVendorTulen.toUpperCase()}";
              }
            } else if (deskripsiBersihDipapar.contains('->')) {
              String bahagianDepan = deskripsiBersihDipapar.split('->').first;
              if (deskripsiMentah.contains('MESSAGE:')) {
                deskripsiBersihDipapar = deskripsiMentah;
              } else {
                deskripsiBersihDipapar = "$bahagianDepan-> ${namaSyarikatVendorTulen.toUpperCase()}";
              }
            }
          ...
          print("Ralat saringan dynamic vendor matching: $e");
        }
      }
      ...
      return _buildHistoryItem(
          ...
          isStatusChange,
          isLast: index == _ticketLogs.length - 1
      );
    }),
  ),
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Container(
                width: 5, height: 5,
                decoration: BoxDecoration(shape: BoxShape.circle, color: active ? color : Colors.grey.shade300),
              ),
              const SizedBox(height: 4),
              Text(
                label,
...
children: [
Theme(
data: ThemeData(iconTheme: const IconThemeData(color: Colors.grey)),
child: Row(
children: [
